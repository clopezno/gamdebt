...
ggplot(data = p3s, aes(x = group_method, y = comment_density, label = ID)) +
  ...
  #stat_summary(fun.data = "mean_cl_boot", colour = "red", size = 0.4)+
...
```{r}
p3s_br <- betareg(formula = comment_density ~ group_method, data = p3s)
```
...
```{r}
p3s_br <- betareg(formula = code_to_test ~ group_method, data = p3s)
```
...
```{r}
p3s_br <- betareg(formula = test_success_density.rescaled ~ group_method, data = p3s)
```
...
```{r}
p3s_br <- betareg(formula = branch_coverage.rescaled ~ group_method, data = p3s)
```
...
```{r}
p3s_br <- betareg(formula = smells_density ~ group_method, data = p3s)
```
...
```{r}
p3s_br <- betareg(formula = sqale_debt_ratio.rescaled ~ group_method, data = p3s)
```
...
```{r}
print("CDp3mean")
meanCDp3 <- tapply(p3s$comment_density,p3s$group_method, mean)
print( meanCDp3)

print("C2Tp3mean")
meanC2Tp3 <- tapply(p3s$code_to_test,p3s$group_method, mean)
print( meanC2Tp3)

print("BCp3mean")
meanTDRp3 <- tapply(p3s$branch_coverage.rescaled,p3s$group_method, mean)
print( meanTDRp3)

print("SDp3mean")
meanSDp3 <- tapply(p3s$smells_density,p3s$group_method, mean)
print( meanSDp3)

print("TDRp3mean")
meanTDRp3 <- tapply(p3s$sqale_debt_ratio.rescaled,p3s$group_method, mean)
print( meanTDRp3)

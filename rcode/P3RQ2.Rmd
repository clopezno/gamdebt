...
# Razonamiento
Evaluar si existen diferencias en las medias proporcionadas por SonarQube en  los diferentes cursos académicos (variable `group_gamification`). En distintos cursos se han utilizado distintos métodos de enseñanza mediante gamificación (delayed,real_time).

Las variables dependientes corresponden a medidas diferentes porcentajes/ratio que deberán ser comparados a través de los diferentes cursos, variable llamada `group_gamification`. Las detalles descriptivos de las medidas pueden obtenerse en la [página de documentación oficial de SonarQube](https://docs.sonarqube.org/latest/user-guide/metric-definitions/).
...
p3s <- read_csv("INPUT/DATA/p3s_completo.csv") %>%
  ...
  mutate(group = factor(group),
         ...
         group_gamification = factor(group_gamification, levels = c("ranking", "live ranking")),
  ...
  filter(group_gamification != 'null')  %>% # eliminación outlayer
...
Para explorar el set de datos `p3s` usaremos la función `summary()` que nos entrega un rsumen numérico dependiente de la clase para cada una de las variables. 
```{r}
summary(p3s)
```
...
```{r}
comment_density <-
ggplot(data = p3s, aes(x = group_gamification, y = comment_density, label = ID)) +
...
```
```{r}
ggsave("comment_density.png",plot=comment_density,path ="./OUTPUT/FIGURES/h2p3/",dpi = 600,scale = 1.5 )
```

#### Regresión beta
La formulación de la *regresión beta* sigue la misma sintaxis que los modelos lineales en R.
```{r}
p3s_br <- betareg(formula = comment_density ~ group_gamification, data = p3s)
```
...
```{r}
ggpredict(p3s_br, terms = "group_gamification")
```
...
pairs(emmeans(p3s_br, "group_gamification"))
```
...
```{r}
code_to_test <-
ggplot(data = p3s, aes(x = group_gamification, y = code_to_test, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
ggsave("code_to_test.png",plot=code_to_test,path ="./OUTPUT/FIGURES/h2p3/",dpi = 600,scale = 1.5 )
```
...
```{r}
p3s_br <- betareg(formula = code_to_test ~ group_gamification, data = p3s)
```
...
<!-- ggplot(data = p3s, aes(x = group_gamification, y = test_success_density, label = ID)) + -->
<!--   geom_boxplot(outlier.size = -1, colour = "gray60") + -->
...
<!-- ggplot(data = p3s, aes(x = group_gamification, y = test_success_density.rescaled, label = ID)) + -->
<!--   geom_boxplot(outlier.size = -1, colour = "gray60") + -->
...
<!-- ```{r} -->
<!-- p3s_br <- betareg(formula = test_success_density.rescaled ~ group_gamification, data = p3s) -->
<!-- ``` -->
...
```{r}
branch_coverage <-
ggplot(data = p3s, aes(x = group_gamification, y = branch_coverage, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
```{r}
branch_coverage
ggsave("branch_coverage.png",plot=branch_coverage,path ="./OUTPUT/FIGURES/h2p3/",dpi = 600,scale = 1)
```
...
```{r}
ggplot(data = p3s, aes(x = group_gamification, y = branch_coverage.rescaled, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
```{r}
p3s_br <- betareg(formula = branch_coverage.rescaled ~ group_gamification, data = p3s)
```
...
```{r}
smells_density <-
ggplot(data = p3s, aes(x = group_gamification, y = smells_density, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
```
```{r}
ggsave("smells_density.png",plot=smells_density,path ="./OUTPUT/FIGURES/h2p3/",dpi = 600,scale = 1.5 )
...
```{r}
p3s_br <- betareg(formula = smells_density ~ group_gamification, data = p3s)
```
...
```{r}
sqale_debt_ratio <-
ggplot(data = p3s, aes(x = group_gamification, y = sqale_debt_ratio, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
ggsave("sqale_debt_ratio.png",plot=sqale_debt_ratio,path ="./OUTPUT/FIGURES/h2p3/",dpi = 600,scale = 1.5 )
```
...
```{r}
ggplot(data = p3s, aes(x = group_gamification, y = sqale_debt_ratio.rescaled, label = ID)) +
  geom_boxplot(outlier.size = -1, colour = "gray60") +
...
```{r}
p3s_br <- betareg(formula = sqale_debt_ratio.rescaled ~ group_gamification, data = p3s)
```
